...
public class SwordAPIEndpoint
{
    ...
    private static Logger log = Logger.getLogger(org.swordapp.server.SwordAPIEndpoint.class);
    ...
    protected String getContentDispositionValue(String disposition, String name)
    {

        String retval = null;
        if (disposition == null || name == null) {
            return null;
        }

        try {
            HeaderTokenizer ht = new HeaderTokenizer(disposition, HeaderTokenizer.MIME);
            HeaderTokenizer.Token token;
            while (ht.peek().getType() != HeaderTokenizer.Token.EOF) {
                token = ht.next();
                if (token.getValue().equals(name)) {
                    token = ht.next();
                    if (token.getValue().equals("=")){
                        retval = ht.next().getValue();
                        break;
                    }
                }
            }
        } catch (Exception e) {
            log.error("Error looking for " + name + " in Content-Disposition header (" + disposition + "): " +
                    e.getMessage());
        }
        if (retval == null) {
            log.error("Error looking for " + name + " in Content-Disposition header (" + disposition + "): null");
        }
        return retval;
    }
